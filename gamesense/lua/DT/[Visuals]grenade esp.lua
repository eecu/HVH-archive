...
for k, v in pairs(sizes) do
	sizes[k][1] = math.floor(v[1] * 0.65)
	sizes[k][2] = math.floor(v[2] * 0.65)
end
                ...
                if entity.is_alive(player) then
                    ...
                    if #player_items[player] > 0 and ui.get(nadeesp) then
                        ...
                        if x1 ~= nil and alpha_multiplier ~= 0 then
                            ...
                            if nade then
                                icons.nade:draw(round(x1 + start + spot), y1 - 35, sizes.nade[1], sizes.nade[2], r, g, b, a, false, "e")
                                spot = spot + 15
                            end
                            if moly then
                                icons.moly:draw(round(x1 + start + spot), y1 - 35, sizes.moly[1], sizes.moly[2], r, g, b, a, false, "e")
                                spot = spot + 15
                            end
                            if incin then
                                icons.incin:draw(round(x1 + start + spot), y1 - 35, sizes.incin[1], sizes.incin[2], r, g, b, a, false, "e")
                                spot = spot + 9
                            end
                            if smoke then
                                icons.smoke:draw(round(x1 + start + spot), y1 - 35, sizes.smoke[1], sizes.smoke[2], r, g, b, a, false, "e")
                            end
                        end
                    end
                end
