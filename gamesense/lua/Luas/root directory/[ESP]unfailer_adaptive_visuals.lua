...
function visuals_main.scope_zoom_overrde:on_paint()
    ...
    if prop == 1 then
        ui.set(self.vars.override_zoom_fov, ui.get(ui_main.main_menu.scope_zoom_override.override_zoom_fov_slider_1))
    elseif prop == 2 then
...
ui_main.main_menu.scope_zoom_override = {
    override_zoom_fov_slider_1 = ui_new_slider("LUA","B", 'Override zoom FOV on first scope', 0, 100, 0, true, '%', 1),
...
local function on_aim_fire(e)
    ...
    if ui_get(ui_main.main_menu.aimlog.aimlog_enable) and ui_get(ui_main.main_menu.aimlog.on_fire_enable) and e ~= nil then
        ...
        "[Unfailer.skeet] 击中了 ", string.lower(target_name),
        ", 击中部位: ", group,
        "  伤害: ", e.damage,
...
local function on_player_hurt(e)
    ...
    if ui_get(ui_main.main_menu.aimlog.aimlog_enable) and ui_get(ui_main.main_menu.aimlog.on_damage_enable) then
        ...
        local message = "[Unfailer.skeet] 击中 " .. string.lower(target_name) .. ", 命中部位: " .. group .. "  伤害: " .. e.dmg_health .. "  剩余血量: " .. rem_health
        if rem_health <= 0 then
            message = message .. " (死)"
        end
...
local function on_aim_miss(e)
    ...
    if ui_get(ui_main.main_menu.aimlog.aimlog_enable) and ui_get(ui_main.main_menu.aimlog.on_miss_enable) and e ~= nil then
        ...
        if e.reason == "?" then
            ...
            client_color_log(r, g, b,
            "[Unfailer.skeet] 正常操作了 ", string.lower(target_name),
            ", 击中部位: ", group,
            "  操作原因: ", reason)
        end
end
...
function main:init_menu()
    ...
    ui_set_visible(ui_main.main_menu.scope_zoom_override.override_zoom_fov_slider_1,select == "Scope zoom override"  and enable)
