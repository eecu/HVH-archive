...
local chinese_data = {
    ...
    ["XZ CT GM HUO"] = "警家拱门",
    ["XZ CT GM YAN"] = "警家拱门",
    ["XZ GM LEI ZUO"] = "警家拱门/左",
    ["XZ GM LEI ZUO2"] = "警家拱门",
    ["XZ GM LEI"] = "警家拱门/里面",
    ["XZ GC YAN"] = "棺材",
    ["XZ GC HUO"] = "棺材",
    ["XZ GCH HUO"] = "棺材后点/教堂门口",
    ["XZ GCH LEI"] = "棺材后点/教堂门口",
    ["XZ LJ YAN"] = "链接",
    ["XZ LJ LEI"] = "链接",
    ["XZ XK HUO"] = "小坑",
    ["XZ DXK YAN"] = "大小坑",
    ...
    ["MIRAGE B 1V1 L MAX"] = "左对角 伤害最大",
    ["MIRAGE B 1V1 L ANTI RUN"] = "左对角 防过点",
    ["MIRAGE B 1V1 R ANTI RUN"] = "右对角 防过点",
    ...
    ["MIRAGE B 1V1 R INWARD"] = "防包点PEEK",
    ["MIRAGE B 1V1 R OUTWARD"] = "防包点过点PEEK",
...
function bs.error_log(T)
    ...
    return print_raw(T), print_dev(T)
end
...
local by = {verdana_bold = render.load_font("verdana", 15, "u")}
local bz = {}
...
local eX = {
    {
        ...
        id = "hvh_locations",
        type = "remote",
        url = "https://raw.githubusercontent.com/DarkLuny/helper-locations/main/hvh_locationss",
        description = "HvH",
        builtin = false
    },
    {
        name = "Legit locations",
        id = "legit_locations",
        type = "remote",
        url = "https://raw.githubusercontent.com/DarkLuny/helper-locations/main/legit.json",
        description = "Legit",
        builtin = false
    }
}
...
function a.source_get_index_data(fy, bu)
    --network.get(fy:gsub("^https://raw.githubusercontent.com/", "https://combinatronics.com/"), function(success, fz)
            local fA = {}
            local _, message1 = pcall(network.get, fy)
            local _, message2 = pcall(network.get, fy:gsub("^https://raw.githubusercontent.com/", "https://combinatronics.com/"))
            local fz
            if message2:find('<head><title>') == nil and message2 ~= '' then
                fz = network.get(fy:gsub("^https://raw.githubusercontent.com/", "https://combinatronics.com/"))
            elseif message1 ~= '' and message2:find('<head><title>') ~= nil then
                fz = network.get(fy)
            else
                fz = '{}'
            end
            --files.write('gmioer.json', fz)
            if not fz or fz:find("404: Not Found") then
                if fz:find("404: Not Found") then
                    bu("404 - Not Found")
                    print_raw("404 - Not Found")
                else
                    bu(string.format("%s", fz ~= '{}'))
                    print_raw(string.format("%s", fz ~= '{}'))
                end
                return
            end
            local fB, fC = pcall(json.parse, fz)
            if not fB then
                bu("Invalid JSON: " .. fC)
        ...
        get_locations = function(self, eU, g3)
            ...
            if f0[self][eU] == nil then
                ...
                elseif fx == nil and g3 and self.type == "remote" and self.url_format ~= nil then
                    local fy = self.url_format:gsub("%%map%%", eU)
                    self.remote_status = string.format("Loading map data for %s...", eU)
                    f1()
                    --http_l.get(fy, function(success, fz)
                    local _, message1 = pcall(network.get, fy)
                    local _, message2 = pcall(network.get, fy:gsub("^https://raw.githubusercontent.com/", "https://combinatronics.com/"))
                    local fz
                    if message1 == '' and message2 ~= '' then
                        fz = network.get(fy:gsub("^https://raw.githubusercontent.com/", "https://combinatronics.com/"))
                    elseif message1 ~= '' and message2 == '' then
                        fz = network.get(fy)
                    else
                        fz = '{}'
                    end
                    if not fz or fz == "404: Not Found" then
                        if fz == "404: Not Found" then
                            self.remote_status = string.format("No locations found for %s.", eU)
                        else
                            self.remote_status =
                                string.format("Failed to fetch %s: %s", eU, fz ~= '{}')
                        end
                        f1()
                        return
                    end
                    local fw, fx = pcall(parse_and_create_locations, fz, eU)
                    if not fw then
                        self.remote_status = string.format("Invalid map data: %s", fx)
                        f1()
                        bs.error_log(
    ...
    for j = 1, #av.sources do
        ...
        if gb.enabled[eZ.id] then
            ...
            for j = 1, #hq do
                ...
                if io and fg.weapons_assoc[bW] and f4 then
...
function a.on_paint()
    ...
    if f4 ~= nil then
                ...
                if jl ~= nil then
                    ...
                    if jn ~= nil then
                        ...
                        for j = 1, #ip do
                            local fg = ip[j]
                            local cn = fg.name
                            cn = tostring(chinese_data[string.upper(cn)] or cn)
                        ...
                        if ip.distance_width_mp > 0 then
                            ...
                            for j = 1, #i3 do
                                ...
                                bz.Text(1, T, vector(jP, jQ), color(s, h, c, b * jj))
                            end
                        end
                        if ib ~= nil then
                ...
                for j = 1, #hP do
                    ...
                    if fg.viewangles_target ~= nil then
                        ...
                        if dJ ~= nil then
                            ...
                            cn = tostring(chinese_data[string.upper(cn)] or cn)
                            local hC
                            local kb = render.measure_text(by.verdana_bold, "au", cn)
                            local kc, kd = kb.x, kb.y
                            local ke, kf = 0, 0
                            if fg.description ~= nil then
                                hC = fg.description:upper():gsub(" ", "  ")
                                local kg = render.measure_text(2, "au", hC .. " ")
                                ke, kf = kg.x, kg.y
                                ke = ke
                            end
        ...
        if iV and hQ ~= nil and (hQ.type == "movement" and iW ~= 'Rage' or hQ.type ~= "movement" and iW == "Legit") then
...
function a.cmd_location_playback_grenade(dg, hI, bW)
    ...
    if a.playback_state == cg then
        if hR.jump then
            ...
            if kW then
                local iW = fL:get()
                if iW == "Rage" and bit.band(dg.buttons, 8) ~= 8 and bit.band(dg.buttons, 16) ~= 16 and bit.band(dg.buttons, 512) ~= 512 and bit.band(dg.buttons, 1024) ~= 1024 then
