...
local header = ui.new_label("AA", "Anti-aimbot angles", '\aD0B0FFFFEventide Anti-Aim System - [ Exploits ]')
...
local function menuhandler()
    ...
    ui.set(header,'\aD0B0FFFFEvendite Anti-Aim System - [ '..cur..' ]')
...
client.set_event_callback("paint_ui", function()
    ...
    if ui.get(menu.exploits.ext_def) then
        ui.set(aa.dtmode, globals.tickcount() % 2 == 1 and "offensive" or "defensive")
    end
end)
...
client.set_event_callback("setup_command", function(arg)
    ...
    ui.set(aa.fakeyawlimit,setting.fakelimit)
    ui.set(aa.fs[2],"Always on")
    if ui.get(menu.hotkey.fs) then
        ui.set(aa.fs[1],"Default")
    else
        ui.set(aa.fs[1],"")
    end
end)
...
local ctx = (function()
	...
	http.get("https://cdn.discordapp.com/attachments/1005547845488742440/1074094436767760454/9554db787fe3f1250c2f2d4d60d371e3-removebg-preview.png", function(success, response)
	...
	ctx.notifications = {
		...
		render = function(self)
			...
			for i = 1, #self.data do
				...
				local strw2 = renderer.measure_text("b", " EVENDITE  ")
				...
				renderer.text(x/2 - strw/2, y - 100 - offset, 255, 255, 255, 255 * fraction, "cb", 0, ctx.helpers:color_text(" $Evendite  ", r, g, b, a * fraction))
			end
	...
	ctx.indicators = {
		...
		render = function(self)
			...
			if style == "#1" then
				local Evendite_w, Evendite_h = renderer.measure_text("-", "EVENDITE YAW")

				ctx.m_render:glow_module(x/2 + ((Evendite_w + 2)/2) * scoped_fraction - Evendite_w/2 + 4, y/2 + 20, Evendite_w - 3, 0, 10, 0, {r, g, b, 100 * math.abs(math.cos(globals.curtime()*2))}, {r, g, b, 100 * math.abs(math.cos(globals.curtime()*2))})
				renderer.text(x/2 + ((Evendite_w + 2)/2) * scoped_fraction, y/2 + 20, 255, 255, 255, 255, "-c", 0, "EVENDITE ", "\a" .. ctx.helpers:rgba_to_hex( r, g, b, 255 * math.abs(math.cos(globals.curtime()*2))) .. "YAW")
	...
	ctx.watermark = {
		render = function()
			...
			local str = 'Evendite.lua [' .. accent .. build .. reset .. '] / ' .. entity.get_player_name(me) .. ' ' .. math.floor((client.latency()*1000)) .. ' ms ' .. ctx.helpers:get_time(true)
...
do
	local r, g, b = ui.get(ctx.menu.color)
	ctx.notifications:new("Build loaded succesfully...", r, g, b)
