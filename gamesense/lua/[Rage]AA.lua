...
local function on_bullet_impact(e)
	...
	if globals.tickcount() - abftimer > 3 and closest < 70 then
		...
		table.insert(paint_funcs.notifications.table_text, {
		text = "[Kazune Yaw] \aFFFFFFFFAnti-Bruteforce Reversed due to enemy shot ",
		timer = globals.realtime(),
...
local enabled_aa = ui.new_checkbox("AA", "Anti-aimbot angles", "\aFB8EEAFF>>[Kazune Yaw] Enable")
--local dangerousmode = ui.new_checkbox("AA", "Anti-aimbot angles", "Dangerous Mode")
local tankaa = ui.new_checkbox("AA", "Anti-aimbot angles", "\a8EA5E5FFExperimental Mode")
local cetou = ui.new_checkbox("AA", "Anti-aimbot angles", "\a8EA5E5FFDangerous Mode")
local onshotfix = ui.new_checkbox("AA", "Anti-aimbot angles", "\a8EA5E5FFOnshot Fakelag Fix")
...
local enable_it = ui.new_checkbox("AA", "Anti-aimbot angles", "\a8EA5E5FFJumpscout")
...
local weaponinfoui = ui.new_checkbox("AA", "Anti-aimbot angles", "\a8EA5E5FFWeapon Info Indicator")
...
table.insert(paint_funcs.notifications.table_text, {
    text = "[Kazune Yaw] \aFFFFFFFF Version - 4.7.4 Update - 2022.5.1",
    timer = globals.realtime(),
...
client.set_event_callback("paint_ui", function(e)
	...
	if ui.get(tankaa) then
		...
		if onground and speed > 5 then
			statetext = 'TANKAA[EX]'
			yawslider = ui.get(highspeed) and 20 or reversed and tankyaw(-40,40) or tankyaw(-42,42) or tankyaw(-44,44)
			...
			roll = ui.get(highspeed) and 45 or 0
			yawfs =	false

		elseif onground then
			statetext = ui.get(cetou) and 'EXTDESYNC' or 'STANDING'
			yawslider = ui.get(cetou) and -23 or ui.get(aa.dt[2]) and tankyaw(-44,45) or -4
			...
			fyaw = ui.get(cetou) and 60 or 23
			roll =  ui.get(cetou) and 45 or 0
			yawfs =	false
		end
		...
		if ui.get(aa.sw[1]) and ui.get(aa.sw[2]) then
			...
			yawslider = ui.get(cetou) and 3 or 0
			jittermode = ui.get(cetou) and 'Off' or 'Center'
			jitterslider = 76
			byawmode = ui.get(cetou) and 'Static' or 'Jitter'
			byawslider = ui.get(cetou) and 180 or 0
			fyaw = ui.get(cetou) and 60 or 58
			roll = ui.get(cetou) and 45 or 0
			yawfs = false
		end
		
		if onground and infiniteduck or onground and ui.get(aa.fakeduck) then
			statetext = ui.get(cetou) and 'EXTDESYNC' or 'DUCKING'
			yawslider = ui.get(cetou) and 45 or tankyaw(-35,36)
			jittermode = ui.get(cetou) and 'Off' or 'Center'
			jitterslider = 0
			byawmode = ui.get(cetou) and 'Static' or 'Static'
		...
		if not ui.get(aa.dt[2]) and not ui.get(aa.os[2]) and infiniteduck then
			statetext = 'DUCKING[CHK]'
			yawslider = -20
			jittermode = 'Offset'
			jitterslider = 69
			byawmode = 'Jitter'
		...
		if client.key_state(0x20) or not onground then
			...
			yawslider = ui.get(cetou) and 7 or 14
			jittermode =  ui.get(cetou) and 'Off' or 'Center'
			jitterslider = ui.get(cetou) and 0 or 73
			byawmode = ui.get(cetou) and 'Static' or 'Jitter'
			byawslider = ui.get(cetou) and 180 or 0
			fyaw = ui.get(cetou) and 60 or 58
			roll = ui.get(cetou) and 45 or 0
		...
		if not ui.get(aa.dt[2]) and not ui.get(aa.os[2]) and not onground then
			statetext = 'INAIR[CHK]'
			yawslider = -5
			jittermode = 'Offset'
			jitterslider = -5
		...
		if onground and infiniteduck or onground and ui.get(aa.fakeduck) then
			...
			yawslider = ui.get(cetou) and 45 or -20
			jittermode = ui.get(cetou) and 'Off' or 'Offset'
			jitterslider = 69
			byawmode = ui.get(cetou) and 'Static' or 'Jitter'
			byawslider = ui.get(cetou) and 180 or 0
			fyaw = 60
	...
	if ui.get(manual_state) == 1 then
		...
		byawslider = -180
		fyaw = 60
		roll = ui.get(cetou) and ui.get(aa.sw[1]) and ui.get(aa.sw[2]) and 45 or 0
		yawfs = false
	end
...
local b = { ui.reference("Rage", "Other", "Double tap") }
...
client.set_event_callback("paint_ui", function()
	...
	--if false then
		...
		renderer.text(menux - 230, menuy + 90, 0, 255, 0, 255, '-', 0, 'V4.7.4')
		renderer.text(menux - 200, menuy + 90, 255, 127, 0, 255, '-', 0, 'DEBUG')
		...
		renderer.text(menux - 270, menuy + 190, 0, 255, 0, 255, '-', 0, '2022.05.01')
