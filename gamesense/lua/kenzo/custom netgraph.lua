...
local function g_paint()
    ...
    surface_draw_text(sp_x - 500 , sp_y + 120, 255, 255, 255, net_state ~= 0 and 255 or alpha, verdana, ccor_text)--clock syncing
    renderer_texture(warning_icon, x - 510, sp_y + 112, 20, 19, --[[ colors ]] color[1], color[2], color[3], color[4])--三角形
    surface_draw_text(x - 480, sp_y + 120, 255, 255, 255, 255, verdana, string_format('+- %s', right_text))--+ ms
    ...
    surface_draw_text(sp_x - 500, sp_y + 140*cn, 255, 255, 255, LC_ALPHA*255, verdana, bytes_in_text);-- in
    surface_draw_text(sp_x + bi_width - 500, sp_y + 140*cn, lerp_clr[1], lerp_clr[2], lerp_clr[3], LC_ALPHA*255, verdana, string_format('lerp: %.1fms', lerp_time)); cn=cn+1--lerp
    --surface_draw_text(sp_x, sp_y + 20*cn, 255, 255, 255, LC_ALPHA*255, verdana, string_format('out: %.2fk/s', net_channel.sent_bytes/1024)); cn=cn+1--out

    surface_draw_text(sp_x - 340, sp_y + 60*cn, 255, 255, 255, LC_ALPHA*255, verdana, string_format('sv: %.2f +- %.2fms    var: %.3f ms', server_framerate, server_var, server_var)); cn=cn+1--sv
    ...
    -- Draw line
     --surface_draw_text(sp_x, sp_y + 20*cn, 255, 255, 255, 255, verdana, tr_text);--tick
    surface_draw_text(sp_x - 340, sp_y + 46.5*cn, pc[1], pc[2], pc[3], LC_ALPHA*255, verdana, nd_text);--delay
    --surface_draw_text(sp_x + nd_width, sp_y + 20*cn, 255, 255 / 100 * additional_latency, 255 / 100 * additional_latency, LC_ALPHA*255, verdana, fl_text); cn=cn+1--datagram
end
