...
local header = ui.new_label("AA", "Anti-aimbot angles", '\aD0B0FFFFAMBANI Anti-Aim System - [ Exploits ]')
...
local function menuhandler()
    ...
    ui.set(header,'\aD0B0FFFFAMBANI Anti-Aim System - [ '..cur..' ]')
...
local ctx = (function()
	...
	ctx.notifications = {
		...
		render = function(self)
			...
			for i = 1, #self.data do
				...
				local strw2 = renderer.measure_text("b", " AMBANI  ")
				...
				renderer.text(x/2 - strw/2, y - 100 - offset, 255, 255, 255, 255 * fraction, "cb", 0, ctx.helpers:color_text(" $AMBANI  ", r, g, b, a * fraction))
			end
	...
	ctx.indicators = {
		...
		render = function(self)
			...
			if style == "#1" then
				local AMBANI_w, AMBANI_h = renderer.measure_text("-", "AMBANI YAW")

				ctx.m_render:glow_module(x/2 + ((AMBANI_w + 2)/2) * scoped_fraction - AMBANI_w/2 + 4, y/2 + 20, AMBANI_w - 3, 0, 10, 0, {r, g, b, 100 * math.abs(math.cos(globals.curtime()*2))}, {r, g, b, 100 * math.abs(math.cos(globals.curtime()*2))})
				renderer.text(x/2 + ((AMBANI_w + 2)/2) * scoped_fraction, y/2 + 20, 255, 255, 255, 255, "-c", 0, "AMBANI ", "\a" .. ctx.helpers:rgba_to_hex( r, g, b, 255 * math.abs(math.cos(globals.curtime()*2))) .. "YAW")
	...
	ctx.watermark = {
		render = function()
			...
			local str = 'AMBANI.lua [' .. accent .. build .. reset .. '] / ' .. entity.get_player_name(me) .. ' ' .. math.floor((client.latency()*1000)) .. ' ms ' .. ctx.helpers:get_time(true)
